...
def generate_v2(input_file, verbose=False):
    # Banner
        ...
        if 'items' in checklist:
            ...
            for item in checklist['items']:
                ...
                if 'subcategory' in item:
                    v2reco['labels'].append({'subarea': item['category']})
                v2reco['queries'] = []
                if 'graph' in item:
...
def store_v2(output_folder, checklist, output_format, verbose=False):
    ...
    for item in checklist:
        ...
        if output_format in ['yaml', 'yml']:
            output_file = os.path.join(output_folder, item['guid'] + ".yaml")
            with open(output_file, 'w') as f:
                yaml.dump(item, f)
            if verbose: print("DEBUG: Stored recommendation in", output_file)
        elif output_format == 'json':
            output_file = os.path.join(output_folder, item['guid'] + ".json")
            with open(output_file, 'w') as f:
                json.dump(item, f)
            if verbose: print("DEBUG: Stored recommendation in", output_file)
        else:
